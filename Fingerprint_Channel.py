...
if __name__ == '__main__':
    for filename1 in os.listdir("channel_data/Channel_breakup_data/"):

      config = get_config()
      path = "channel_data/Channel_breakup_data/"+filename1+"/"

      # fingerprint all files in a directory

      for filename in os.listdir(path):
        if filename.endswith(".mp3"):
         if(os.path.getsize(path+filename)<>346L):
          file_name="20_mins/"+filename[-6]+filename[-5]+"/fingerprints2.db"
          ...
          print colored('   new channel, going to analyze..', 'green')
