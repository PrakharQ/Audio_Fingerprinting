...
if __name__ == '__main__':
  ...
  filename="/home/prakhar/finger/mp3/"
  filename2=raw_input()
  file_obj = FileReader(filename+filename2,5)
  file_obj.parse_audio()
  #print(file_obj.filename)
  #print(file_obj.out_channels)
  #print(data.shape)
  data = file_obj.out_channels
  #print data
  ...
  def find_matches(samples, Fs=fingerprint.DEFAULT_FS):
    hash = fingerprint.fingerprint(samples, Fs=Fs)
    hash=list(hash)
    with open('daemons.txt', 'w') as fp:
        fp.write('\n'.join('%s %s' % x for x in hash))
    with open("daemons.txt",'r') as f:
        mylist=[tuple([i.split(' ')[0],int(i.split(' ')[1])]) for i in f]
    hashes =mylist
    #print type(hashes)
    #rint hashes
